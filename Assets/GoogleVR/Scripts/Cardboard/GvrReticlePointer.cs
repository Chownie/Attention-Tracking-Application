...
public class GvrReticlePointer : GvrBasePointer {
  ...
  public const float RETICLE_MIN_INNER_ANGLE = 0.3f;

  /// Minimum outer angle of the reticle (in degrees).
  public const float RETICLE_MIN_OUTER_ANGLE = 0.8f;

  public const float RETICLE_MAX_INNER_ANGLE = 1.5f;

  public const float RETICLE_MAX_OUTER_ANGLE = 2f;
  ...
  public void UpdateDiameters() {
    ...
    if (ReticleInnerAngle > RETICLE_MAX_INNER_ANGLE) {
      ReticleInnerAngle = RETICLE_MAX_INNER_ANGLE;
    }

    if (ReticleOuterAngle > RETICLE_MAX_OUTER_ANGLE) {
      ReticleOuterAngle = RETICLE_MAX_OUTER_ANGLE;
    }
    ...
    if(ReticleInnerDiameter < inner_diameter) {
      reticleGrowthSpeed = 100f;
    } else {
      reticleGrowthSpeed = 0.5f;
    }

    if(CurrentRaycastResult.gameObject != null) {
          if(CurrentRaycastResult.gameObject.tag == "board") {
      CurrentRaycastResult.gameObject.SendMessage("FocusPoint", CurrentRaycastResult.worldPosition);
    }

    if(ReticleOuterDiameter < 0.02 && CurrentRaycastResult.gameObject != null) {
      Target targ = CurrentRaycastResult.gameObject.GetComponent<Target>();
      if(targ != null) {
        CurrentRaycastResult.gameObject.GetComponent<Target>().OnGazeLock();
      }
    }
    }
  ...
  private bool SetPointerTarget(Vector3 target, bool interactive) {
    ...
    if (interactive) {
      ReticleInnerAngle = RETICLE_MAX_INNER_ANGLE - RETICLE_GROWTH_ANGLE;
      ReticleOuterAngle = RETICLE_MAX_OUTER_ANGLE - RETICLE_GROWTH_ANGLE;
    } else {
      ReticleInnerAngle = RETICLE_MAX_INNER_ANGLE;
      ReticleOuterAngle = RETICLE_MAX_OUTER_ANGLE;
    }
